...
const DriverLoginScreen: React.FC<DriverLoginScreenProps> = ({ onNavigate }) => {
    ...
    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        alert('Driver login successful! Navigating to dashboard...');
        ...
        <div className="min-h-screen bg-[#0a2a66] font-sans flex flex-col items-center justify-center p-4 relative">
            ...
            <main className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-8 space-y-6 z-10">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 mt-1">Welcome back, partner!</p>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="flex items-center justify-between text-sm">
                        <button type="button" onClick={() => onNavigate('driverRegistration')} className="font-medium text-[#0a2a66] hover:text-[#082250]">
                            Create Account
                        </button>
                        <button type="button" onClick={() => onNavigate('driverPasswordRecovery')} className="font-medium text-[#0a2a66] hover:text-[#082250]">
                            Forgot Password?
                        </button>
                    </div>
